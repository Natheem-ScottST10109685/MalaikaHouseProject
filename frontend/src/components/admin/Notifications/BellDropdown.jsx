...
export default function BellDropdown({ onViewAll }) {
  ...
  async function loadLatest() {
    const res = await apiFetch("/admin/notifications?limit=10");
    if (res.ok) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-96 bg-white rounded-xl shadow-lg border p-2 z-50">
          <div className="flex items-center justify-between px-2 py-1">
            <div className="font-semibold text-slate-700">Notifications</div>
            <div className="flex gap-2">
              <button className="text-xs px-2 py-1 rounded bg-gray-100 hover:bg-gray-200" onClick={markAllRead}>
                Mark all read
              </button>
              <button className="text-xs px-2 py-1 rounded bg-indigo-600 text-white hover:bg-indigo-700" onClick={() => { setOpen(false); onViewAll?.(); }}>
                View all
              </button>
            </div>
          </div>
