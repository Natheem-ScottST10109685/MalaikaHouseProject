    ...
    <Modal open={open} onClose={onClose} title="Student Report" footer={footer}>
      <div className="space-y-4">
        <div className="bg-slate-50 rounded-lg p-3 text-sm">
          ...
          <div><span className="text-slate-500">Created by:</span> <span className="text-slate-800">{report.createdBy?.email ?? "—"}</span></div>
          <div><span className="text-slate-500">Created at:</span> <span className="text-slate-800">{new Date(report.createdAt).toLocaleString()}</span></div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-white">
          ...
          <div className="border rounded-lg p-3">
            ...
            <div className="text-3xl font-bold text-[#7B9BC4]">{report.progressScore ?? "—"} / 10</div>
          </div>
        </div>
